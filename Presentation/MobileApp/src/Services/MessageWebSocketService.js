...
class MessageWebSocketService {
    ...
    async initialize() {
        try {
            ...
            const baseURL = "http://10.62.201.105:5297"; // Backend IP từ Api.js
    ...
    setupEventHandlers() {
        ...
        this.connection.on("MessageSent", (message) => {
            console.log(
                "[MessageWebSocket] Message sent confirmation:",
                message
            );
    ...
    async sendMessage(
        ...
        if (
            ...
            console.warn(
                "[MessageWebSocket] Cannot send - not connected, reconnecting..."
            );
            await this.connect();
    ...
    async markAsRead(conversationId) {
        ...
        if (
            ...
            console.warn(
                "[MessageWebSocket] Cannot mark as read - not connected"
            );
    ...
    async getOnlineUsers() {
        ...
        if (
            ...
            console.warn(
                "[MessageWebSocket] Cannot get online users - not connected"
            );
            return; // Silently fail
        }
    ...
    handleReconnect() {
        ...
        console.log(
            `[MessageWebSocket] Reconnecting in ${delay}ms (attempt ${this.reconnectAttempts})`
        );
    ...
    emit(event, data) {
        ...
        this.listeners[event].forEach((callback) => {
            try {
                ...
                console.error(
                    `[MessageWebSocket] Listener error for event ${event}:`,
                    error
                );
            }
        });
