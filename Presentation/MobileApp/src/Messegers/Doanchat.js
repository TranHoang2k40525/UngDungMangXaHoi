...
export default function Doanchat() {
  ...
  renderCount.current++;
  console.log(`[Doanchat] RENDER #${renderCount.current} - userId: ${userId}, currentUserId: ${currentUserId}`);
  ...
  const loadConversation = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        console.log(`[Doanchat] RESPONSE: Got ${response.data.messages.length} messages (expected: ${pageSize}), total: ${totalMessages}, pages: ${totalPages}`);
  ...
  useEffect(() => {
    if (conversationDetail?.conversation_id) {
      ...
      const markAsReadWhenReady = async () => {
        ...
        if (isConnected) {
          console.log('[Doanchat] Marking conversation as read:', conversationDetail.conversation_id);
          MessageWebSocketService.markAsRead(conversationDetail.conversation_id);
        } else {
          console.warn('[Doanchat] Cannot mark as read - WebSocket not connected');
        }
      };
          ...
          {loading || !currentUserId ? (
            ...
            <View style={styles.messagesContainer}>
              {messages.map((msg, index) => {
                ...
                // Debug logging
                if (index === 0) {
                  console.log(`[Doanchat] Rendering messages - currentUserId: ${currentUserId}`);
                }
                console.log(`[Doanchat] Message #${index}: sender_id=${msg.sender_id}, isOwnMessage=${isOwnMessage}, content="${msg.content.substring(0, 20)}"`);
                    ...
                    {!isOwnMessage && (
                      userAvatar ? (
                        ...
                        <View style={[styles.messageAvatar, styles.defaultAvatar]}>
                          <Text style={[styles.defaultAvatarText, { fontSize: 12 }]}>
                            {userName ? userName.charAt(0).toUpperCase() : 'U'}
                          </Text>
                        </View>
                      )
