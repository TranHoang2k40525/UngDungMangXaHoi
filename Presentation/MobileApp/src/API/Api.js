...
const apiCall = async (endpoint, options = {}) => {
    try {
        console.log(
            `[API-CALL] Making API call to: ${API_BASE_URL}${endpoint}`
        );
            ...
            if (
                ...
                try {
                    ...
                    if (storedRefresh) {
                        ...
                        if (rfRes.ok) {
                            const newAccess =
                                rfJson?.AccessToken || rfJson?.accessToken;
                            const newRefresh =
                                rfJson?.RefreshToken || rfJson?.refreshToken;
...
const normalizeUri = async (uri) => {
    ...
    if (Platform.OS === "ios" && uri.startsWith("ph://")) {
        try {
            ...
            console.warn(
                "[API] Could not convert iOS ph:// URI, no localUri found"
            );
            return uri;
        } catch (e) {
...
/**
 * Helper để đảm bảo tên file luôn có đuôi mở rộng hợp lệ
 * @param {string} uri - URI của file
 * @param {string} currentName - Tên file hiện tại (có thể không có đuôi)
 * @param {string} defaultExtension - Đuôi mở rộng mặc định (ví dụ: '.jpg', '.mp4')
...
export const getMyGroups = async () => {
    try {
        ...
        const response = await fetch(
            `${API_BASE_URL}/api/groupchat/my-groups`,
            {
                method: "GET",
                headers: {
                    ...headers,
...
export const getSharesByPost = async (postId) => {
    try {
        const response = await fetch(
            `${API_BASE_URL}/api/shares/post/${postId}`,
            {
                method: "GET",
                headers: {
                    Accept: "application/json",
