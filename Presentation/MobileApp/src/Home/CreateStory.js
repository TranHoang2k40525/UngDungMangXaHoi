...
export default function CreateStory() {
  ...
  const isVideo = media && (media.type?.toLowerCase?.().includes('video') || String(media.name || '').toLowerCase().endsWith('.mp4'));
    ...
    try {
      ...
      try {
        ...
        if (userStr) {
          ...
          ownerId = u?.user_id ?? u?.userId ?? u?.UserId ?? u?.id ?? null;
          const raw = u?.avatarUrl || u?.avatar_url || null;
          if (raw) {
            avatarSrc = String(raw).startsWith('http') ? { uri: raw } : { uri: `${API_BASE_URL}${raw}` };
          }
        }
      } catch (e) { console.warn('Read userInfo failed', e); }

      const mediaType = isVideo ? 'video' : 'image';
      const res = await createStory({ media, mediaType, privacy, userId: ownerId });
  ...
  return (
    <View style={[styles.container, { paddingTop: (insets.top || 0) + 12 }] }>
      ...
      <View style={styles.privacyRow}>
        <TouchableOpacity style={[styles.privacyBtn, privacy==='public' && styles.privacyBtnActive]} onPress={() => setPrivacy('public')}>
          <Text style={[styles.privacyText, privacy==='public' && styles.privacyTextActive]}>Công khai</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.privacyBtn, privacy==='followers' && styles.privacyBtnActive]} onPress={() => setPrivacy('followers')}>
          <Text style={[styles.privacyText, privacy==='followers' && styles.privacyTextActive]}>Người theo dõi</Text>
        </TouchableOpacity>
        <TouchableOpacity style={[styles.privacyBtn, privacy==='private' && styles.privacyBtnActive]} onPress={() => setPrivacy('private')}>
          <Text style={[styles.privacyText, privacy==='private' && styles.privacyTextActive]}>Riêng tư</Text>
        </TouchableOpacity>
      </View>

  <View style={[styles.actionsRow, { paddingBottom: (insets.bottom || 0) + 12 }]}>
...
const styles = StyleSheet.create({
  ...
  previewContainer: { flex: 1, marginBottom: 12, borderRadius: 12, overflow: 'hidden', backgroundColor: '#000' },
  media: { width: '100%', height: '100%' },
  empty: { justifyContent: 'center', alignItems: 'center' },
  privacyRow: { flexDirection: 'row', justifyContent: 'space-between', marginVertical: 12 },
  privacyBtn: { flex: 1, padding: 10, marginHorizontal: 6, borderRadius: 8, backgroundColor: '#f3f4f6', alignItems: 'center' },
  privacyBtnActive: { backgroundColor: '#111827' },
  privacyText: { color: '#111827', fontWeight: '600' },
  privacyTextActive: { color: '#fff' },
  actionsRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' },
  cancelBtn: { paddingVertical: 12, paddingHorizontal: 18, borderRadius: 8, backgroundColor: '#f3f4f6' },
  cancelText: { color: '#111827', fontWeight: '600' },
  uploadBtn: { paddingVertical: 12, paddingHorizontal: 18, borderRadius: 8, backgroundColor: '#111827' },
