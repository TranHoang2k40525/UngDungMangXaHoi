...
const ANDROID_VISUAL_SELECTED =
    PermissionsAndroid?.PERMISSIONS?.READ_MEDIA_VISUAL_USER_SELECTED ||
    "android.permission.READ_MEDIA_VISUAL_USER_SELECTED";

// Helper to convert ph:// URIs to file:// URIs
const getMediaInfo = async (asset) => {
    try {
        if (Platform.OS === "ios" && asset.uri.startsWith("ph://")) {
            // Get the actual file info for iOS ph:// URIs
        ...
        console.log(`${LOG} Error getting asset info:`, error);
...
export default function CreatePost() {
        ...
        (async () => {
            ...
            try {
                if (Platform.OS === "android") {
                    console.log(
                        `${LOG} ANDROID initial: checking current permissions`
                    );
                ...
                if (current.canAskAgain !== false) {
                    console.log(
                        `${LOG} requestPermissionsAsync (initial iOS ask) ...`
                    );
                    const ask = await MediaLibrary.requestPermissionsAsync();
                    console.log(
                        `${LOG} requestPermissionsAsync (initial iOS result) ->`,
                        {
                            status: ask?.status,
                    ...
                    console.log(
                        `${LOG} iOS cannot ask again, need to open Settings`
                    );
                }
                ...
                if (Platform.OS === "android") {
                    Alert.alert(
                        ...
                        "Vui lòng mở Cài đặt và cấp quyền Ảnh/Video cho ứng dụng. Sau đó quay lại màn hình này."
                    );
                } else {
                    // iOS: Lỗi không mong đợi, hướng dẫn mở Settings
                    Alert.alert(
                        "Không thể yêu cầu quyền",
                        "Vui lòng mở Cài đặt hệ thống để cấp quyền Thư viện/Ảnh cho ứng dụng."
                    );
                }
                if (mounted) setLoading(false);
            }
    ...
    const androidCheckMediaPermissions = useCallback(async () => {
        ...
        try {
            ...
            if (sdk >= 33) {
                ...
                const hasVideos = await PermissionsAndroid.check(
                ...
                const hasAudio = PermissionsAndroid.PERMISSIONS.READ_MEDIA_AUDIO
                    ...
                    : true;
                const hasVisualSelected = await PermissionsAndroid.check(
                    ANDROID_VISUAL_SELECTED
                ).catch(() => false);
                console.log(`${LOG} Android check ->`, {
                    hasImages,
                    hasVideos,
                    hasAudio,
                    hasVisualSelected,
                ...
                console.log(
                    `${LOG} Android check (READ_EXTERNAL_STORAGE) ->`,
                    hasStorage
                );
    ...
    const openSystemPicker = useCallback(async () => {
        try {
            ...
            const result = await ImagePicker.launchImageLibraryAsync({
                ...
                quality: 1,
                videoQuality:
                    ImagePicker.UIImagePickerControllerQualityType.High, // Chất lượng video cao
    ...
    const handleRequestPermissions = useCallback(async () => {
        try {
            if (Platform.OS === "android") {
                console.log(
                    `${LOG} ANDROID manual: open settings instead of requesting`
                );
            ...
            if (cur.canAskAgain === false) {
                console.log(
                    `${LOG} iOS cannot ask again, opening settings directly`
                );
                Alert.alert(
                    "Cần cấp quyền trong Cài đặt",
                    "Ứng dụng cần quyền truy cập Ảnh để bạn có thể đăng bài. Vui lòng mở Cài đặt > " +
                        (Platform.OS === "ios"
                            ? "Quyền riêng tư"
                            : "Ứng dụng") +
                        ' > Ảnh và chọn "Tất cả ảnh".',
                    [
                                ...
                                try {
                                    if (
                                        ...
                                        showPermissionSuccess(
                                            "Đã cập nhật ảnh được chọn."
                                        );
                                    } else {
                                        Alert.alert(
                                            "Không hỗ trợ",
                                            "Phiên bản iOS hiện tại không hỗ trợ chọn thêm ảnh."
                                        );
                                    }
                                } catch (err) {
                                    console.log(
                                        `${LOG} presentPermissionsPickerAsync error`,
                                        err
                                    );
                                }
                            },
            ...
            console.log(
                `${LOG} requestPermissionsAsync (manual iOS result) ->`,
                {
                    status: res?.status,
            ...
            if (
                ...
                if (res.accessPrivileges === "limited") {
                    showPermissionSuccess(
                        "Đã cấp quyền (Ảnh được chọn). Bạn có thể sử dụng thư viện."
                    );
                } else {
                    showPermissionSuccess(
                        "Đã cấp quyền. Bạn có thể chọn ảnh/video."
                    );
                }
                return;
            }
            ...
            if (res.status !== "granted" && res.canAskAgain === false) {
                console.log(
                    `${LOG} iOS denied and cannot ask again after manual request`
                );
                Alert.alert(
                    "Cần cấp quyền trong Cài đặt",
            ...
            Alert.alert(
                "Lỗi",
                "Không thể yêu cầu quyền. Vui lòng mở Cài đặt để cấp quyền cho ứng dụng."
            );
        }
    ...
    const loadAssets = async (reset = false) => {
        ...
        if (Platform.OS === "android") {
            ...
            if (!ok) {
                console.log(
                    `${LOG} loadAssets aborted: no permission (android)`
                );
                return;
            }
        } else {
        ...
        try {
                ...
                res.assets.map(async (asset) => {
                    if (
                        ...
                        try {
                            ...
                            console.log(
                                `${LOG} Error converting asset ${asset.id}:`,
                                error
                            );
                            return asItem(asset);
                        }
            ...
            setAssets((prev) =>
                reset ? processedAssets : [...prev, ...processedAssets]
            );
            ...
            if (reset && processedAssets.length > 0) {
                setSelectedImage(processedAssets[0]);
            }
        } catch (error) {
            console.log(`${LOG} Error loading assets:`, error);
            Alert.alert("Lỗi", "Không thể tải ảnh/video. Vui lòng thử lại.");
        }
    };
    ...
    const filteredAssets = useMemo(() => {
        if (selectedTab === "Photo")
            return assets.filter(
                (a) => a.mediaType === "photo" || a.mediaType === "image"
            );
    ...
    const handleSelectImage = async (item) => {
        // Ensure we have the correct URI before selecting
        let processedItem = item;
        if (Platform.OS === "ios" && item.uri.startsWith("ph://")) {
            try {
                ...
                console.log(`${LOG} Error processing selected item:`, error);
            }
        }

        const isItemVideo =
            processedItem.mediaType === "video" ||
            processedItem.type === "video";

        if (multipleSelectMode) {
            const hasVideoSelected = selectedItems.some(
                (i) => i.mediaType === "video" || i.type === "video"
            );

            if (isItemVideo) {
                if (processedItem.duration && processedItem.duration > 60) {
                    Alert.alert(
                        "Video quá dài",
                        `Video này dài ${Math.round(
                            processedItem.duration
                        )}s. Vui lòng chọn video dưới 60 giây.`
                    );
                    return;
                }
                setSelectedItems([processedItem]);
                setSelectedImage(processedItem);
            } else {
                if (hasVideoSelected) {
                    setSelectedItems([processedItem]);
                    setSelectedImage(processedItem);
                    ...
                    if (isSelected) {
                        ...
                        setSelectedItems([...selectedItems, processedItem]);
                        setSelectedImage(processedItem);
                    }
                }
            }
    ...
    return (
        <SafeAreaView edges={["top"]} style={styles.container}>
                    ...
                    selectedImage.type === "video" ? (
                        <>
                            ...
                            {selectedImage.duration &&
                                selectedImage.duration > 60 && (
                                    <View style={styles.warningBanner}>
                                        <Text style={styles.warningText}>
                                            ⚠️ Video quá dài (
                                            {Math.round(selectedImage.duration)}
                                            s). Giới hạn 60s.
                                        </Text>
                                    </View>
                                )}
            ...
            {(
                Platform.OS === "ios"
                    ? permStatus !== "granted" && iosAccess !== "limited"
                    : permStatus !== "granted" && androidAccess !== "limited"
            ) ? (
                <View style={styles.permOverlay}>
                    <View style={styles.permCard}>
                        <Text style={styles.permTitle}>
                            Cần quyền truy cập ảnh và video
                        </Text>
                        ...
                        {Platform.OS === "ios" && (
                            <Text style={styles.permHint}>
                                💡 Nếu chọn "Ảnh được chọn", bạn vẫn có thể sử
                                dụng ứng dụng với các ảnh/video đã chọn.
                            </Text>
                        )}
                            ...
                            {Platform.OS === "android" ? (
                                <>
                                    ...
                                    <TouchableOpacity
                                        onPress={async () => {
                                            const ok =
                                                await androidCheckMediaPermissions();
                                            console.log(
                                                `${LOG} Android check (manual recheck) ->`,
                                                ok
                                            );
                                            if (ok) {
                                                setPermStatus("granted");
                                                ...
                                                Alert.alert(
                                                    "Chưa có quyền",
                                                    "Vui lòng cấp quyền trong Cài đặt cho Expo Go rồi thử lại."
                                                );
                                            }
                                        }}
                                        style={[
            ...
            showLimitedOverlay ? (
                <View style={styles.limitedBanner}>
                    <Text style={styles.limitedText}>
                        Đang dùng quyền "Ảnh được chọn". Bạn có thể thêm
                        ảnh/video hoặc chuyển sang "Tất cả ảnh".
                    </Text>
                    <View style={styles.limitedActions}>
                            ...
                            onPress={async () => {
                                try {
                                    if (
                                        ...
                                        Alert.alert(
                                            "Không hỗ trợ",
                                            'Phiên bản iOS hiện tại không hỗ trợ chọn thêm ảnh trong chế độ hạn chế. Vui lòng mở Cài đặt để chuyển sang "Tất cả ảnh".'
                                        );
                                    }
                                } catch (err) {
                                    console.log(
                                    ...
                                    Alert.alert(
                                        "Lỗi",
                                        "Không thể mở bộ chọn ảnh. Vui lòng thử lại hoặc mở Cài đặt."
                                    );
                                }
                            }}
                            style={[
                        ...
                        >
                            <Text
                                style={[styles.permBtnText, { color: "#fff" }]}
                            >
                                Chọn thêm ảnh…
                            </Text>
                        </TouchableOpacity>
                        ...
                        >
                            <Text
                                style={[styles.permBtnText, { color: "#111" }]}
                            >
                                Mở Cài đặt
                            </Text>
                        </TouchableOpacity>
                        ...
                        >
                            <Text
                                style={[styles.permBtnText, { color: "#333" }]}
                            >
                                Ẩn
                            </Text>
                        </TouchableOpacity>
            ...
            {Platform.OS === "android" &&
            androidAccess === "limited" &&
            showLimitedOverlay ? (
                <View style={styles.limitedBanner}>
                    <Text style={styles.limitedText}>
                        Đang ở quyền "Truy cập bị hạn chế". Bạn có thể tiếp tục
                        dùng, chọn ảnh/video từ thiết bị hoặc mở Cài đặt để
                        chuyển sang "Luôn cho phép tất cả".
                    </Text>
                    <View style={styles.limitedActions}>
                        <TouchableOpacity
                            onPress={openSystemPicker}
