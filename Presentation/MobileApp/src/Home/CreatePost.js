...
import { Image } from "expo-image"; // Use expo-image for iOS ph:// URI support
import {
  SafeAreaView,
  useSafeAreaInsets,
...
export default function CreatePost() {
  ...
  // Update video source when selectedImage changes (handle iOS ph:// URIs)
  useEffect(() => {
    if (
      selectedImage &&
      (selectedImage.mediaType === "video" || selectedImage.type === "video")
      ...
      const loadVideo = async () => {
        try {
          ...
          console.warn("[CreatePost] Failed to load video:", e);
        }
      };
      ...
      if (cur.canAskAgain === false) {
        ...
        Alert.alert(
          ...
          "Ứng dụng cần quyền truy cập Ảnh để bạn có thể đăng bài. Vui lòng mở Cài đặt > " +
            (Platform.OS === "ios" ? "Quyền riêng tư" : "Ứng dụng") +
  ...
  const loadAssets = async (reset = false) => {
    ...
    setAssets((prev) =>
      reset ? res.assets.map(asItem) : [...prev, ...res.assets.map(asItem)]
    );
  ...
  return (
    ...
    <SafeAreaView edges={["top"]} style={styles.container}>
      ...
      <View style={styles.previewContainer}>
                ...
                {selectedImage.duration && selectedImage.duration > 60 && (
                  <View style={styles.warningBanner}>
                    <Text style={styles.warningText}>
                      ⚠️ Video quá dài ({Math.round(selectedImage.duration)}s).
                      Giới hạn 60s.
                    </Text>
                  </View>
                )}
        ...
        >
          <View style={styles.selectMultipleIcon}>
            {multipleSelectMode && <View style={styles.selectMultipleCheck} />}
          </View>
          ...
          <View style={styles.permCard}>
            ...
            {Platform.OS === "ios" && (
              <Text style={styles.permHint}>
                💡 Nếu chọn "Ảnh được chọn", bạn vẫn có thể sử dụng ứng dụng với
                các ảnh/video đã chọn.
              </Text>
            )}
            <View style={styles.permActions}>
              {Platform.OS === "android" ? (
                <>
                  ...
                  >
                    <Text style={[styles.permBtnText, { color: "#111" }]}>
                      Kiểm tra lại
                    </Text>
                  </TouchableOpacity>
                </>
      ...
      showLimitedOverlay ? (
        <View style={styles.limitedBanner}>
          <Text style={styles.limitedText}>
            Đang dùng quyền “Ảnh được chọn”. Bạn có thể thêm ảnh/video hoặc
      ...
      showLimitedOverlay ? (
        <View style={styles.limitedBanner}>
          <Text style={styles.limitedText}>
            Đang ở quyền “Truy cập bị hạn chế”. Bạn có thể tiếp tục dùng, chọn
            ảnh/video từ thiết bị hoặc mở Cài đặt để chuyển sang “Luôn cho phép
            tất cả”.
          </Text>
          <View style={styles.limitedActions}>
            <TouchableOpacity
