...
export default function PostDetail() {
  ...
  const initPostStates = async (postsArray) => {
    ...
    for (const p of postsArray) {
      try {
        ...
        let sharesCount = 0;
        try {
          const sharesResponse = await getSharesByPost(p.id);
          const sharesData = sharesResponse?.data || sharesResponse || [];
  ...
  const loadUserPosts = async (page = 1, append = false) => {
    ...
    try {
      ...
      if (data && Array.isArray(data)) {
        const newStates = await initPostStates(data); // Now async!
  ...
  const handleShareToFeed = async (postId, caption, privacy) => {
    try {
      ...
      // Reload actual shares count from API
      try {
        const sharesResponse = await getSharesByPost(postId);
        const sharesData = sharesResponse?.data || sharesResponse || [];
        const actualSharesCount = Array.isArray(sharesData)
          ? sharesData.length
          : 0;
        ...
        setPostStates((prev) => {
          ...
          return { ...prev, [postId]: { ...cur, shares: actualSharesCount } };
        });
      } catch (err) {
        setPostStates((prev) => {
          const cur = prev[postId] || {
          ...
          return (
            <View style={styles.post}>
              ...
              <View style={styles.postStats}>
                ...
                <TouchableOpacity
                  onPress={() => onOpenReactions(post.id)}
                  activeOpacity={0.7}
                  style={{
                    flexDirection: "row",
                    alignItems: "center",
                ...
                >
                  ...
                  {state.topReactions && state.topReactions.length > 0 && (
