...
const formatVietnameseTime = (timestamp) => {
  ...
  try {
    ...
    if (!isUTC) {
      ...
      console.log(
        "[formatVietnameseTime] No Z, added 7 hours:",
        timestamp,
        "→",
        vietnamCommentTime.toISOString()
      );
    } else {
...
const CommentsModal = ({
    ...
    const loadCurrentUser = async () => {
      try {
        ...
        if (userStr) {
          ...
          console.log(
            "[CommentsModal] 👤 Parsed user object:",
          ...
          const raw =
            user?.UserId ?? user?.userId ?? user?.user_id ?? user?.id ?? null;
          ...
          if (Number.isFinite(uidNum)) {
            ...
            console.warn(
              "[CommentsModal] ⚠️ Could not extract valid userId from userInfo"
            );
          }
        } else {
    ...
    if (highlightCommentId && comments.length > 0 && flatListRef.current) {
      ...
      if (index !== -1) {
        setTimeout(() => {
          ...
          // Highlight comment trong 2 giây
  ...
  useEffect(() => {
        ...
        try {
          ...
          commentService.onReceiveComment((c) => {
            try {
              ...
              console.error(
                "[CommentsModal] onReceiveComment handler error",
                e
              );
            }
          });
          ...
          commentService.onCommentUpdated((c) => {
            try {
              ...
              console.error(
                "[CommentsModal] onCommentUpdated handler error",
                e
              );
            }
          });
          ...
          commentService.onCommentDeleted((payload) => {
            try {
              ...
              console.error(
                "[CommentsModal] onCommentDeleted handler error",
                e
              );
            }
          });
          ...
          commentService.onCommentReplyAdded((payload) => {
            try {
              ...
              console.error(
                "[CommentsModal] onCommentReplyAdded handler error",
                e
              );
            }
          });
    ...
    return () => {
      (async () => {
        ...
        try {
          commentService.removeAllListeners();
        } catch (e) {
          /* ignore */
        }
      })();
      ...
      if (editingComment) {
        ...
        const newCommentData = {
          ...
          parentId: response?.parentCommentId
            ...
            : replyingTo.id, // Dùng từ backend hoặc fallback
        };
        ...
        const newCommentData = {
          ...
          parentId: response?.parentCommentId
            ? String(response.parentCommentId)
            : null, // Phải là null cho comment gốc
        };
  ...
  const handleNavigateToProfile = (comment) => {
    ...
    setTimeout(() => {
      ...
      if (isMyComment) {
        ...
        console.log("[CommentsModal] Navigate to UserProfilePublic:", {
  ...
  const handleMentionPress = async (username) => {
    ...
    setTimeout(async () => {
      ...
      if (!userId) {
        console.log(
          "[CommentsModal] 🔍 User not in comments, searching by username..."
        );
        try {
  ...
  const renderComment = ({ item }) => {
    ...
    return (
      ...
      >
        <View style={[isHighlighted && styles.highlightedComment]}>
      ...
      >
              ...
              {(replyingTo || editingComment) && (
                <View style={styles.replyBanner}>
                  <View style={styles.replyBannerContent}>
                    <Ionicons
                      name={
                        editingComment ? "create-outline" : "arrow-undo-outline"
                      }
                      size={16}
                      color="#8E8E8E"
                    />
        ...
        {showMenuForComment
          ? (() => {
              ...
              const commentUserId =
                comment.userId != null ? Number(comment.userId) : null;
              const isOwner =
                currentUserId != null &&
                commentUserId != null &&
              ...
              console.log("[CommentsModal] 🔐 Menu ownership check:", {
                ...
                isOwner: isOwner,
                comparison: `${commentUserId} === ${currentUserId} = ${
                  commentUserId === currentUserId
                }`,
              });
