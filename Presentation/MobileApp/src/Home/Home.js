        ...
        <View style={{ position: "relative" }}>
            <FlatList
                ...
                onMomentumScrollEnd={(e) => {
                    const w =
                        e.nativeEvent.layoutMeasurement.width || imageWidth;
...
export default function Home() {
    ...
    const [showReactionsListPostId, setShowReactionsListPostId] =
        useState(null);
    ...
    const normalizeUser = (u) => {
        if (!u)
            return { id: null, username: "", avatarUrl: null, fullName: "" };
        const rawId =
            u?.id ?? u?.userId ?? u?.user_id ?? u?.UserId ?? u?.user_id;
        const id = Number(rawId);
        const username =
            u?.username ?? u?.userName ?? u?.user_name ?? u?.name ?? "";
        const rawAvatar =
            u?.avatarUrl ?? u?.avatar_url ?? u?.userAvatar ?? null;
    ...
    const loadFeedStories = async () => {
        try {
            ...
            console.log(
                "[Home] Unique users after deduplication:",
                uniqueUsers.size
            );
            ...
            const mapped = Array.from(uniqueUsers.values()).map((u) => {
                ...
                const hasStory =
                    stories.length > 0 || !!u.storyId || !!u.mediaUrl;
            ...
            console.log(
                "[Home] Final mapped friends with stories:",
                mapped.length
            );
        ...
        (async () => {
            try {
                ...
                try {
                    ...
                    if (profId != null) {
                        ...
                        if (Number.isFinite(uid)) {
                            ...
                            console.log(
                                "[HOME] getProfile() -> userId set:",
                                uid
                            );
                        } else {
                            console.log(
                                "[HOME] getProfile() -> invalid userId:",
                                profId
                            );
                        }
                    } else {
                        console.log(
                            "[HOME] getProfile() -> no userId on payload"
                        );
                    }
                } catch (e) {
                    console.log(
                ...
                if (mounted) {
                    ...
                    console.log(
                        `[HOME] 📥 Initial feed loaded - ${arr.length} posts`
                    );
                    ...
                    for (const p of arr) {
                        try {
                            console.log(
                                `[HOME] Loading reactions for post ${p.id}...`
                            );
                            const reactionData = await getReactionSummary(p.id);
                            console.log(
                                `[HOME] Post ${p.id} reaction data:`,
                                reactionData
                            );
                            const topReactions = (
    ...
    useEffect(() => {
        ...
        const connectSignalR = async () => {
            try {
                ...
                notificationSignalRService.onReceiveNotification(
                    (notification) => {
                        console.log(
                            "[HOME] New notification received:",
                            notification
                        );
    ...
    const loadUnreadNotificationCount = async () => {
        try {
            ...
            console.error(
                "[HOME] Load unread notification count error:",
                error
            );
        }
    };
    ...
    const handleReaction = async (postId, reactionType) => {
        ...
        try {
            ...
            try {
                ...
                console.log(
                    `[HOME] ✅ getReactionSummary success:`,
                    reactionData
                );
        ...
        try {
            ...
            for (const p of arr) {
                ...
                try {
                    ...
                    console.error(
                        `[HOME REFRESH] ❌ Error for post ${p.id}:`,
                        err
                    );
                    next[p.id] = {
                        liked: false,
                                    ...
                                    if (images.length === 1) {
                                        return (
                                            <TouchableOpacity
                                                ...
                                                onPress={() => {
                                                    if (
                                                        typeof setSingleViewerUrl ===
                                                            "function" &&
                                                        typeof setSingleViewerVisible ===
                                                            "function"
                                                    ) {
                                                        setSingleViewerUrl(
                                                            images[0].url
                                                        );
                                                        setSingleViewerVisible(
                                                            true
                                                        );
                                                    } else if (
                        ...
                        <View style={styles.postActions}>
                            <View style={styles.postActionsLeft}>
                                <View
                                    ref={(ref) => {
                                        if (ref)
                                            likeButtonRefs.current[p.id] = ref;
                                    }}
                                    collapsable={false}
                                    style={{
                                        flexDirection: "row",
                        ...
                        <View style={styles.postStats}>
                            ...
                            >
                                {postStates[p.id]?.topReactions?.length > 0 && (
                                    ...
                                    >
                                        {postStates[p.id].topReactions.map(
                                            (type, idx) => (
                                                <Text
                                                    key={idx}
                                                    style={{
                                ...
                                >
                                    <Text style={styles.likeCount}>
                                        {(
                                            postStates[p.id]?.likes ?? 0
                                        ).toLocaleString()}{" "}
                                        lượt thích •{" "}
                                        {(
                                            postStates[p.id]?.shares ?? 0
                                        ).toLocaleString()}{" "}
                                        lượt chia sẻ
                                    </Text>
                                </TouchableOpacity>
            ...
            <CommentsModal
                ...
                onCommentAdded={async (postId) => {
                    try {
                        ...
                        console.error(
                            "[Home] Update comment count error:",
                            error
                        );
                    }
                }}
            ...
            {showTagListPostId && (
                ...
                >
                    ...
                    >
                        ...
                        {Array.isArray(tagListForModal) &&
                        tagListForModal.length > 0 ? (
                ...
                >
                    ...
                    >
                        ...
                        <Text style={{ color: "#666", marginBottom: 8 }}>
                            Nhấn ✕ trên chip để gỡ; nhấn + để thêm người chưa
                            được gắn
                        </Text>
                        ...
                        >
                            {(editTagsList || []).map((t) => (
                                ...
                                >
                                    ...
                                    >
                                        <Text style={styles.tagChipCloseText}>
                                            ✕
                                        </Text>
                                    </TouchableOpacity>
                                </View>
                        ...
                        >
                            ...
                            <TouchableOpacity
                                ...
                                onPress={() =>
                                    submitEditTags(showEditTagsPostId)
                                }
                            >
